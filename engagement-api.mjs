import {engagementEndpoint} from './site-config.mjs';
...
export async function readArticle(article){
  if(!engagementEndpoint)throw Error('unavailable');
  const url=new URL(engagementEndpoint);url.searchParams.set('article',article);if(visitor())url.searchParams.set('visitor',visitor());
  const response=await fetch(url,{signal:AbortSignal.timeout(25000)}),data=await response.json();if(!data.ok)throw Error('unavailable');return data;
}
